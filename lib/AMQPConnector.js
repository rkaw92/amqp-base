 ...
 * @constructor
 * @param {(string|string[])} serverURI The AMQP URI of the broker which the connector should connect to, in the format: amqp://user:password@host.tld:port/vhost
 */
function AMQPConnector(serverURI, socketOptions) {
...
AMQPConnector.prototype._createConnection = function _createConnection() {
	...
	when.try(amqplib.connect, URI, self._socketOptions).done(function connectionSucceeded(connection) {
